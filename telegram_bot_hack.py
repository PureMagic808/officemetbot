#!/usr/bin/env python
"""
Особый файл для запуска Telegram-бота через workflow run_bot.
"""
import os
import sys
import logging
...
# Если этот скрипт вызывается с аргументом command=run_bot, запускаем бота
if len(sys.argv) > 1 and sys.argv[1] == "run_bot":
    ...
    # Если выполняется без аргументов, запустим сами себя но с правильным аргументом
    logger.info("Перезапуск скрипта с аргументом run_bot...")
    subprocess.run([sys.executable, sys.argv[0], "run_bot"])