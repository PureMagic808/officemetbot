#!/usr/bin/env python3
"""
Специальный скрипт для прямого запуска только Telegram-бота через workflow run_bot.
Этот файл минимизирован и содержит только самое необходимое для запуска бота.
"""
import os
import sys
import random
...
logger.info("Запуск бота через run_bot_direct.py...")
...
try:
    ...
    logger.error("Не удалось импортировать библиотеки Telegram!")
    sys.exit(1)
...
user_states: Dict[int, Dict[str, Union[Set[int], Dict[int, int]]]] = {}
...
async def send_random_meme(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Отправляем мем в зависимости от его типа
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.callback_query is None:
        logger.error("Не удалось получить данные о нажатой кнопке!")
        return
    ...
    if user_id in user_states:
        ...
        user_states[user_id]["ratings"] = ratings
