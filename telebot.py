#!/usr/bin/env python
"""
Самостоятельный запуск Telegram-бота для workflow run_bot
"""
import os
import sys
import logging
...
async def send_random_meme(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    while not suitable_meme_found and attempts < max_attempts:
...
if __name__ == "__main__":
    logger.info("Запуск бота из файла telebot.py")
    try:
        main()
    except Exception as e:
        logger.error(f"Необработанная ошибка при запуске бота: {e}")
        import traceback
        logger.error(traceback.format_exc())