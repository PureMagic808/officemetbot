...
def is_suitable_meme_advanced(meme):
    ...
    try:
        ...
        if not text or len(text.split()) < 2:
            logger.info(f"Мем отфильтрован: отсутствует или слишком короткий текст")
            return False
        ...
        # Список слов, намекающих на юмористический/офисный контент
        meme_keywords = ["офис", "работа", "босс", "начальник", "коллега", "коллеги", 
                        "сотрудник", "проект", "отпуск", "зарплата", "понедельник",
                        "совещание", "собрание", "дедлайн", "срок", "пятница", "выходной"]
        
        has_meme_keyword = any(keyword in text for keyword in meme_keywords)
        
        # Если нет ни одного ключевого слова о работе и офисе, это подозрительно для нашей тематики
        if not has_meme_keyword and len(text.split()) < 10:
